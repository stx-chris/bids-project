...
# Optional: nur Top 10 signifikante
top_enrichment <- enrichment_up_sora |> 
...
ggplot(top_enrichment, aes(x = reorder(Term, -`adj.P.Val`), y = -log10(`adj.P.Val`))) +
  ...
  coord_flip() +
  labs(
    title = "Top Enriched Hallmark Pathways (Sorafenib UP)",
    x = "Pathway",
    y = "-log10(adj. p-value)"
  ) +
  theme_minimal()

enrichment_down_sora <- hyperG(hallmark, up_tram, universe, cutoff = 0.05, adj.P.Val = TRUE) # empty???
```

#### Gemeinsamkeiten: Durch beide Medikamente runter-regulierte Hallmark Gensets

```{r}
enrichment_up_tram <- hyperG(hallmark, up_tram, universe, cutoff = 0.05, adj.P.Val = TRUE)
