...
author: 
  ...
  - "Christoph Kovacs <christoph.kovacs@master-bids.de>"
format:
  html:
    toc: true
...
Die Datenverarbeitung, -analyse und Interpretation erfolgte in Zusammenarbeit mit Constanze Leichtweiß, Ivanna Marchesini und Enrico Wondra (Gruppe 3). 

Für die Analyse und Verifizierung wurde stellenweise künstliche Intelligenz eingesetzt.
:::

# Code Repository

Der Quellcode zu dieser Arbeit ist hier hinterlegt: <a href="https://github.com/stx-chris/bids-project" data-preview-link>Github Christoph Kovacs</a>
...
sum(keep) # keeping 15,805 records
sum(dim(count_matrix)[1]-sum(keep)) # discarding 41,968 records
dds <- dds[keep,] 
```

::: {.callout-important icon=false}
## Interpretation
Es verbleiben 15.805 Gene für die weitere Analyse. Als Filterkriterium wurden nur diejenigen Gene behalten, bei denen in mindestens 2 Samples mindestens 10 Reads vorhanden sind. Dabei wurden 41.968 Records herausgefiltert.
:::
...
::: {.callout-important icon=false}
## Interpretation
Modellanpassung ist gelungen, da:

- die rote Linie gut durch die Punktwolke läuft,
- keine massiven systematischen Abweichungen sichtbar sind.
- Ausreißer-Dispersionswerte (blaue Kreise) ausreichend angepasst sind (um stabilere Tests zu ermöglichen)

Nach Anwendung von Shrinkage (blaue Punkte) wurden viele extreme log₂ Fold Changes bei niedrig exprimierten Genen korrigiert.

- Die geschrumpften Effekte sind robuster und besser interpretierbar.
- Gene mit hohen Counts und stabil hohen oder niedrigen log₂ Fold Changes bleiben auch nach Shrinkage deutlich sichtbar.
:::
...
plot_counts_trans <- df_raw %>%
  ...
  geom_boxplot(outlier.size = 0.3) +
  theme_bw() +
  labs(title = "Log2-transformierte Counts pro Bedingung",
       x = "Bedingung", y = "Counts (trans)") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
plot_counts_trans
...
plot_counts_normtrans <- df_norm %>% 
  ...
  geom_boxplot(outlier.size = 0.3) +
  theme_bw() +
  labs(title = "Log2-transformierte und normalisierte Counts pro Bedingung",
...
::: {.callout-important icon=false}
## Interpretation
Die transformierten und normiert+transformierten Verteilungen sind nun nahezu identisch zwischen den Proben:

- Die Mediane und die Verteilungen der Counts sind angeglichen worden.
- Die Normalisierung war erfolgreich, d. h. systematische Unterschiede zwischen den Proben (z. B. durch Sequenziertiefe) wurden entfernt.
- Erst jetzt sind verlässliche Vergleiche zwischen Bedingungen (DMSO, Sorafenib, Trametinib) möglich.
:::
...
Da PCA und MDS nur lineare Abhängigkeiten berücksichtigen, werden noch zwei nicht-lineare Techniken verwendet:
1. t-SNE: t-distributed stochastic neighbor embedding for non-linear relationship analysis; bildet lokale Zusammenhänge gut ab
2. UMAP: Uniform Manifold Approximation and Projection, bildet lokale und globale Zusammenhänge gut ab
...
plot_deg <- ggplot(all_counts, aes(x = Comparison, y = n, fill = Regulation)) +
  ...
  labs(title = "Anzahl signifikanter DEGs (FDR < 0.05)", x = "Behandlungsgruppe", y = "Anzahl Gene") +
  theme_minimal()
plot_deg
...
Man erkennt, dass Sorafenib mit rund 2000 hoch- und 500 runterregulierten Genen deutlich mehr Wirkung erzielt als Trametinib mit rund 1500 hoch- und 200 runterregulierten Genen
:::
...
Wirkmechanismus (molekular):

- Hemmung von Proliferation: über Runterregulation von MYC, FOSL1, CDK-Stimulation
- Förderung von Apoptose: durch Hochregulation von BMF
- Zellzyklus-Stillstand: über CDKN1B (p27)
- Hemmung von Invasion/Metastasierung: über TNS4-Downregulation
- mögliche metabolische Umstellung: über PCK1
:::
...
plot_volcano_tram <- EnhancedVolcano(res_tram,
    lab = gene_reference$Symbol[match(rownames(res_tram),as.character(gene_reference$ENSEMBL))],
    x = 'log2FoldChange',
    y = 'padj',
    pCutoff = 0.05,
    FCcutoff = 1,
...
Trametinib dämpft gezielt MAPK-abhängige Onkogenexpression (FOSL1), blockiert Stress-Überlebenssignale (IER3) und hemmt Invasionsmechanismen (TNS4). Das entspricht exakt seiner pharmakologischen Wirkung als MEK-Inhibitor.
:::
...
Obschon Sorafenib und Trametinib eine hohe Anzahl gemeinsam hochregulierter Gene aufweisen (219), ist die isolierte Wirkung von Sorafenib mit 1213 Genen deutlich höher als von Trametinib alleine (12).
:::
...
Bei der Runterregulierung zeigt sich ein ähnliches Bild wie zuvor. Mit 486 gemeinsam runterregulierten Genen teilen Sorafenib und Trametinib einen guten Teil ihrer Wirkung. Doch Sorafenib ist mit 1437 isoliert runterregulierten Genen deutlich wirkungsstärker als Trametinib mit nur 29 isoliert runterregulierten Genen
:::
...
**Transkriptionelle Veränderungen durch Sorafenib**
...
**Transkriptionelle Veränderungen durch Trametinib**
...
::: {.callout-important icon=false}
## Beschreibung
Mithilfe einer Gene Ontology (GO) Analyse untersuchen wir die biologischen Funktionen, Prozesse und zellulären Komponenten, mit denen die Gene des Datensets assoziiert ist. Ziel ist es, zu verstehen, welche biologischen Themen überrepräsentiert sind — also funktionelle Anreicherungen (enrichments) zu identifizieren.
:::
