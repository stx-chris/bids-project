...
sum(keep) # keeping 23,938 records
sum(dim(count_matrix)[1]-sum(keep)) # discarding 33'835 records
dds <- dds[keep,] 
```
...
# collapse technical replicates? (necessary?)
...
# compare contrasts vs baseline model
resultsNames(dds)
results(dds, contrast = c("group", "Sorafenib", "DMSO"))
results(dds, contrast = c("group", "Trametinib", "DMSO"))
results(dds, contrast = c("group", "Sorafenib", "Trametinib"))

plotMA(res) # look in the right upper and lower areas for significant outliers
